...
opts{2}.invariants.time.size = 2^15;
opts{2}.invariants.time.T = 2^14;
opts{2}.invariants.time.subscripts = 1;
opts{3}.invariants.time.size = 2^15;
...
S2psi_refs = generate_refs(S{1+2}{1,1}.data, 1, S{1+2}{1,1}.ranges{1+0});
for S2psiref_index = 1:size(S2psi_refs, 2)
    S{1+2}{1,1}.data = subsasgn( ...
        S{1+2}{1,1}.data, S2psi_refs(:, S2psiref_index), ...
        S2psiref_index);
end
S2phi_refs = generate_refs(S{1+2}{1,2}.data, 1, S{1+2}{1,2}.ranges{1+0});
for S2phi_ref_index = 1:size(S2phi_refs, 2)
    S{1+2}{1,2}.data = subsasgn( ...
        S{1+2}{1,2}.data, S2phi_refs(:, S2phi_ref_index), ...
        S2phi_ref_index);
end
...
networks = [ ...
    ...
    repmat(2, size(S2psi_refs, 2), 1) ;  ...
    repmat(3, size(S2phi_refs, 2), 1)];
...
for ref_index = 1:nS2psi_refs
    ...
    gamma1_index = S2psi_refs(3,ref_index).subs{2};
    gamma1_range = ...
    ...
    thetagamma_index = S2psi_refs(3,ref_index).subs{3};
