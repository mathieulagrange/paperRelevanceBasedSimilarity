...
For a small number of clusters ($M = 1$ or $M = 2$), all methods perform worse, since not enough discriminative sound objects are extracted from the recording.
This corresponds a summary statistics model.
...
As the number of clusters increases, the \emph{RbQ-a} method performs significantly worse for both scattering features and MFCCs since any distinct objects are averaged out by clusters representing the background.
