...
Within a supervised setting of acoustic scene classification (ASC), we have outlined the comparative benefits of log-scattering features over MFCCs, of majority voting at a mid-level time scale of $T=372\,\mathrm{ms}$ over prediction at a high-level time scale of $T=24\,\mathrm{ms}$, and of an adapted Gaussian kernel over the linear kernel in support vector machine (SVM) classification.
