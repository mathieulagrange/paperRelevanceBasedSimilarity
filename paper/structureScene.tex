...
The similarity between the scene centroids $\mu_i$ in the entire dataset, where $i=\lbrace1,2,\ldots,I \rbrace$ for $I=U \times M$, is computed using a radial basis function (RBF) kernel $K$ combined with the local scaling method proposed in \cite{selfTuneManor2004}:
\begin{equation}
\label{eq:kc}
K_{ij} = \exp\left( - \dfrac{\Vert \mu_i - \mu_j \Vert^2}{\Vert \mu_i - \mu_{k,i} \Vert \Vert \mu_j - \mu_{k,j} \Vert} \right).
\end{equation} 
Here, $\mu_{k,i}$ and $\mu_{k,j}$ are the $k^{\textrm{th}}$ nearest neighbor to the centroids $\mu_i$ and $\mu_j$, respectively, and $\Vert \cdot \Vert$ denotes the Euclidean norm. \ja{Wouldn't it be clearer to define $K$ using scene indices $u$ and $v$ and feature indices $m$ and $n$?}

To compute the similarity between two scenes, we then consider several centroid-based similarity metrics:
\begin{itemize}
\item \emph{ob-closest} (\emph{ob-c}): the similarity between two scenes is equal to the largest similarity between their centroids,
\item \emph{ob-averaged} (\emph{ob-a}): the similarity between two scenes is equal to the average of their centroid similarities, and
