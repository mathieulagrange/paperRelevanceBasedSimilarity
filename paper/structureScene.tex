...
Representing the scene at larger time scale as a set of examplars of coherent segments is an effective approach for scene similarity retrieval as it allows the retrieval system to focus on discriminative features. This benefit is reduced in the supervised scene classification, probably because much of the discriminability potential of the approach is already learned by the classifier. \ja{This last part is very vague and hard for me to understand. Why is this the case?}
\end{abstract}
...
Representign a scene using a reduced set of exemplars, in the form of centroids, with adapted similarity strategies allows us to improve ASSR performance compared to the insufficient BOF approaches based on GMMs. These results advocate for a hierarchical model of the scene, where short-time structure -- \ie below a second -- are represented using a convolutional network such as the scattering transform, whereas larger time scales -- \ie over a second -- are modeled as adaptive aggregates, identified in the time domain in the unsupervised setting and in the features domain in the supervised case, provided that the dimensionality is sufficiently high. \ja{I'm not sure what this last part means.}

This assertion will be studied further in future work by considering more complex convolutional networks, larger databases and more diverse applications, such as pleasantness prediction \cite{lafaySoundscapePilot, lafay:hal-01111782} and other ecoacoustics tasks.
