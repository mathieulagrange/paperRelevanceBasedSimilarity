...
The \emph{RbQ} measures are compared to commonly used early integration approach \emph{early}, which consists in averaging over time the feature set of each scene, resulting in one feature vector per scene. The distance on this average feature vector is then used to determine $p@k$. For the BOF approach of Aucouturier et al. \cite{aucouturier2007bag}, $10$-component GMMs are estimated for each scene using the expectation-maximization (EM) algorithm \cite{dempster-laird}. The similarity between a given pair of scene GMMs is then calculated through Monte Carlo sampling approximation. To ensure convergence of the EM algorithm for the scattering features, we reduce their dimension from $1367$ to $30$ by projecting the features onto the top $30$ principal components of the dataset.
...
As seen in Figure \ref{fig:ASS_-1}, the BOF approach behaves similarly to the early approach when applied to MFCCs. In contrast, its performance decreases compared to the early approach when applied to scattering features. Extensive analysis of those results is out of the scope of this paper, but this likely results from the dimension reduction applied prior to estimating the GMMs in the BOF model.
...
The lines should be thicker and follow a color palette of complementary colors (blue vs. orange). The legends should not be abbreviated: scatT -> scattering, mfcc -> MFCC. } \ja{Should we not have $k$ stricly smaller than $10$ here?}}
\label{fig:ASS_-1}
\end{center}
\end{figure}
...
Figure \ref{fig:ASS_0} shows how logarithmic compression is beneficial irrespective of the similarity model used. The gain is more pronounced when considering alternative similarities. \ja{What's ``alternative'' similarities?}
