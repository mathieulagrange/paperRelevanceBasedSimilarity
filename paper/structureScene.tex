...
where $\lbrace f_{ij} \rbrace$ is the optimal flow between the clusters weights $w_i^u$ and $w_j^v$, that is, the amount transported from the $i$th bin to the $j$th bin. $D_{ij}$ is the ground distance between the centroids $c_i^u$ and $c_j^v$, which is equal to $D_{ij}=1-K^c_{ij}$. As suggested in \cite{pele2009fast}, the $\alpha$ parameter is set to one.\\
...
The influence of the temporal-scattering transform and the \emph{ob} are assessed in an unsupervised setting (\emph{i.e.} ASS) and in a supervised setting (\emph{i.e.} ASC). 
...
As most of the 2013 DCASE challenge participants did, a Support Vector Machine (SVM), which is a large-margin binary discriminators with tunable regularization of misclassified examples, is used as classifier for the ASC setting. For the \emph{ob} approach, \gl{TODO}. For both the \emph{ei} and \emph{li} approaches, a linear kernel as well as a RBF kernel are used. As for the \emph{ob} approach, the RBF kernel is scaled locally using the method proposed in \cite{selfTuneManor2004}. All features, \emph{i.e.} the frames, the pooled frames or the centroids depending on the approach considered, are standardized prior to be filled into the classifier. The standardization is computed with respect to the train/test set split of the cross validation scheme (see Section \ref{sec:stand}).
