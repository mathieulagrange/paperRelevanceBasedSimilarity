...
\hline
\cite{phan2016label} & 84 to 87 & Label Tree Embedding of Gammatone, SVMs \\
\cite{7362846} & 85$^*$ & AMS, LDA \\
\end{tabular}
\end{center}
\caption{Algorithms evaluated on the DCASE2013 ASC DCASE2013 dataset. RQA stands for recurrence quantification analysis, ILD for inter-aural differences, AMS for amplitude Modulated Spectrum and UL for unsupervised learning. Algorithms above the horizontal line participated to the DCASE challenge. Others used the private dataset afterward and the last used the public dataset.  \label{tab:dcase}}
\end{table}


In addition, it is still challenging, with a state-of-the-art system based on feature design \ja{?} and SVMs achieving $76\%$ \cite{roma2013} (winner of the DCASE2013 challenge). Another system, using a HOG+SVM approach \cite{rakotomamonjy2015histogram}, achieves $75\%$ and $92\%$ on the DCASE2013 and Rouen datasets, respectively. Recent advances using hierarchical classifier architectures obtain between $84\%$ and $87\%$ on DCASE2013 depending on the prior knowledge used during training \cite{phan2016label}. On the Rouen dataset, the state-of-the-art is currently $95\%$ \cite{bisot2016acoustic}.
...
as shown in Figure \ref{folds}, scattering coefficients outperform MFCCs over all but one fold of the DCASE 2013 private dataset when applying early integration, and over all folds when applying late integration.
