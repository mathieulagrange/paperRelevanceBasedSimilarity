...
% In particular, the inter-intensity difference (IID) between left and right channel conveys the approximate azimuth of each sound source that make up the scene with respect to the listener.
Yet, since the location of the recordist may vary across instances of the same class, the feature space for classification should be invariant to the location of sources.
In order to achieve invariance to location, averaging the left and right channels into a monophonic signal is by far the most widespread technique.

\begin{equation}
\boldsymbol{x}(t) =
\alpha \boldsymbol{\times x_\mathsf{L}}\!(t) +
(1-\alpha) \boldsymbol{\times x_\mathsf{R}}\!(t)
\end{equation}
In practice, $\alpha$ takes values in ${0\%, 25\%, 50\%, 75\%, 100\%}$.

\subsection{Feature selection}

\subsection{Logarithmic compression}
% linearizes the dependency with respect to the distance to the source
