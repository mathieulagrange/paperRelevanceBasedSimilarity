...
This section reviews \ml{several means of achieving those goals:} binaural data augmentation, feature selection, logarithmic compression, standardization, and temporal integration.
\ml{The former has, to the best of our knowledge, not being considered for ASC and specific to multichannel recordings}, whereas the next ones would also apply to many other kinds of data.
...
In order to achieve invariance to ILD, averaging the left and right channels into a monophonic signal is by far the most widespread approach \ml{for classification purposes}.
However, it favors the center of the scene while attenuating lateral cues and sources that are on distinct azimuths get mixed, which may cause bias at training time.
