...
In addition, representing the scene at larger time scale as a set of representative features from coherent segments is an effective approach for scene similarity retrieval as it allows the retrieval system to focus on discriminative features.
\end{abstract}
...
Most state-of-the-art systems more or less follow the bag-of-frames (BOF) approach introduced by Aucouturier et al. \cite{aucouturier2007bag}, where a scene is modeled by high-level summary statistics computed from local features.
...
Whether we adopt a time-frequency perspective or a machine learning perspective on the problem of acoustic scene classification, such operations would either be qualifies as post-processing or pre-processing.
