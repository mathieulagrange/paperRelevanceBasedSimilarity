...
\node (histlabel1) at (2.5, 0.01) {$\mathbf{S}\boldsymbol{x}$};
\draw[-{Latex[length=2mm]}] (-0.3, 0.01) -- (histlabel1);
\draw[-{Latex[length=2mm]}] (-0.3, 0.01) -- (-0.3, 2.5);
\node at (1, -0.35) {(a)};
\node (histlabel2) at (7.8, 0.01) {$\log \mathbf{S}\boldsymbol{x}$};
\node[anchor=south west,inner sep=0] at (3.3, 0) {\includegraphics[height=2cm,width=3.5cm]{feature_histogram_logcompressed.png}};
