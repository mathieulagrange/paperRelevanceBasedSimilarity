...
dimensionless frequency $1$ and bandwidth $1/Q_1$.
A filter bank of wavelets is built by dilating $\boldsymbol{\psi}(t)$
according to a geometric sequence of temporal scales $2^{-\gamma_1/Q_1}$.
The variable $\gamma_1$, akin to a log-frequency, takes integer values between $0$ and $(J_1 \times Q_1 - 1)$.
In the sequel, the number of filters per octave $Q_1$ is set to $8$, and
the number of octaves $J_1$ is set to $10$, \ie the maximum range of human hearing.
We denote by
\begin{equation}
...
has a center frequency of $2^{-\gamma_1/Q_1}$, a bandwidth of $2^{-\gamma_1/Q_1}/Q_1$, and a quality factor of $Q_1$.
...
\end{equation}
The wavelet scalogram bears a resemblance to the constant-Q transform (CQT),
