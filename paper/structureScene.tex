...
As studied in \cite{lagrange:hal-01082501}, the DCASE2013 dataset has interesting intra class diversity while remaining manageable in terms of size, making it suitable for extensive evaluation of algorithmic design choices. Also, it is still challenging as state-of-the-art systems based on feqtures design and support vector machines (SVMs) achieves 76 \% \cite{roma2013} (winner of the 2013 challenge RQA+SVM). For the sake of comparison the HOG+SVM approach \cite{rakotomamonjy2015histogram} achieves 75 \% and 92 \% on the dcase2013 and the Rouen datasets respectively. On the former, recent advances using hierarchical classifiers architectures reports 84 to 87 \% of accuracy depending on the prior knowledge considered for building the classifier architecture \cite{phan2016label}. On the latter, the state-of-the-art is currently 95 \% \cite{bisot2016acoustic}. 
...
\vl{we should discuss color schemes and axis labels} \ml{yes we should but for the second submission, go ahead}}
\end{center}
\end{figure}
...
\begin{center}
\begin{tabular}{c|cc}
             & time-scattering & mfcc \\
             \hline
early int.   & XX (XX)    & XX (XX)   \\
late int.    & XX (XX)    & XX (XX)   \\
ob-closest.  & XX (XX)    & XX (XX)   \\
ob-averaged. & XX (XX)    & XX (XX)   \\
ob-weighted. & XX (XX)    & XX (XX)   \\
\end{tabular}
\caption{...}
\end{center}
\end{table}
